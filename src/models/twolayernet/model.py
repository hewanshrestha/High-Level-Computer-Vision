...
class TwoLayerNetv2(TwoLayerNetv1):
    ...
    def compute_loss(self, X, y=None, reg=0.0):
        ...
        #############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        data_loss = -1/N*np.sum(np.multiply(self.one_hot(y,scores.shape[1]), np.log(scores)))
    ...
    def one_hot(self, y, num_output_nodes):
      #one hot
      v = np.zeros((len(y), num_output_nodes))
...
class TwoLayerNetv3(TwoLayerNetv2):
    ...
    def back_propagation(self, X, y=None, reg=0.0):
        ...
        #############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        dz3 = -1./N*(self.one_hot(y,scores.shape[1]) - scores)
        dw2 =  np.dot(a2.T, dz3) + 2*reg*W2
        db2 =  np.sum(dz3, axis = 0, keepdims = True)
        da2 = np.dot(dz3, W2.T)
        dz2 = np.multiply(da2, self.relu(z2, derivative=True))
        dw1 = np.dot(X.T,dz2) + 2*reg*W1
