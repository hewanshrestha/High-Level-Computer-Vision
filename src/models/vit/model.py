...
class EncoderBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        residual = input
        result = self.ln_1(residual)
        result, attention_weights = self.self_attention(result, result, result, need_weights=self.need_weights)
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            ...
            layers[f"encoder_layer_{i}"] = EncoderBlock(num_heads, hidden_dim, mlp_dim, norm_layer, self.need_weights) # Replace None with Block
            #################################
    ...
    def forward(self, input: torch.Tensor):
        ...
        result = input + self.pos_embedding  # Add positional embedding
...
class VisionTransformer(nn.Module):
    ...
    def visualize_cls(self, attention_weights, n_h, n_w):
        ...
        # Take out the attention weights for the CLS token
        cls_attention_weights = attention_weights[:, 0, 1:]

        # Reshape the attention weights into a 2D grid
        cls_attention_weights = cls_attention_weights.reshape(-1, n_h, n_w)

        # Add a channel dimension for the 2D attention map
        visualized_attention = cls_attention_weights.unsqueeze(-1)

        return visualized_attention
