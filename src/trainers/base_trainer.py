    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self._do_evaluate():
                ...
                val_metric.append(eval_result[self.monitor_metric[-4:]])
            ...
            if self.monitor_mode != 'off' : # Then there is a metric to monitor
                if self.monitor_metric in log: # Then we have measured it in this epoch
                    ...
                    # if improved, save the model checkpoint

                    if self.monitor_mode == "max" and len(val_metric) == np.argmax(val_metric)+1:                        
                        path = os.path.join(self.checkpoint_dir, f'best_model_max.pth')
                        self.save_model(path=path)
                        print(f"Validation Set {self.monitor_metric} increased at epoch: {self.current_epoch}. Saving Model.....")
                        early_stop_count = 0  #set early_stop_count to 0 since we have improvement


                    if self.monitor_mode == "min" and len(val_loss) == np.argmin(val_loss)+1:
                        path = os.path.join(self.checkpoint_dir, f'best_model_min.pth')
                        self.save_model(path=path)
                        print(f"Validation Set {self.monitor_metric} decreased at epoch: {self.current_epoch}. Saving Model.....")
                    ...
                    if self.monitor_mode == "max" and len(val_metric) != np.argmax(val_metric)+1:
                        ...
                        if early_stop_count == self.early_stop:
                            print(f"Validation Set {self.monitor_metric} did not increase for {self.early_stop} epochs. Stopping Training.....")
                            break   
                    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
