...
class MLPTrainer(BaseTrainer):
    ...
    def _train_epoch(self):
        ...
        for batch_idx, (images, labels) in enumerate(self._train_loader):
            ...
            outputs = self.model(images.reshape(-1,self.model.layers[0].weight.shape[1]))
            ...
            self.epoch_metrics.update('loss', self.loss.item())
            for metric in self.metric_ftns:
                self.epoch_metrics.update(str(metric), metric.compute(outputs, labels))

            pbar.set_description(f"Train Epoch: {self.current_epoch} Loss: {self.loss.item():.6f}")
    ...
    def evaluate(self, loader=None):
        ...
        for batch_idx, (images, labels) in enumerate(loader):
            ...
            ####################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            output = self.model(images.reshape(-1,self.model.layers[0].weight.shape[1]))
            # _ , output = torch.max(output, 1)
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****

            self.writer.set_step((self.current_epoch - 1) * len(loader) + batch_idx, 'valid')
            self.eval_metrics.update('loss', self.loss.item())
            for metric in self.metric_ftns:
                self.eval_metrics.update(str(metric), metric.compute(output, labels))

            pbar.set_description(f"Val Loss: {self.loss.item():.6f}")
            self.writer.add_image('input', make_grid(images.cpu(), nrow=8, normalize=True))
